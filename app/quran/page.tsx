...
const Quran = () => {
  ...
  const useFilter = (key: string) => {
    ...
    const newValue = allSurah?.filter(
      (item) =>
        item.namaLatin.toLocaleLowerCase().includes(key.toLocaleLowerCase()) ||
        String(item.nomor).includes(String(key))
    );
  ...
  return (
    <main>
      ...
      <section
        id="surah"
        className="bg-white min-h-[380px] rounded-t-3xl mt-5 flex flex-col items-center p-8"
      >
        {surah && surah?.length > 0 ? (
            ...
            {surah?.map((item) => {
              return (
                <Link key={item.nomor} href={`detail/${item.nomor}`}>
                  ...
                  >
                    <div className="bg-green grid grid-cols-12 p-2 cursor-pointer rounded-xl border-2 border-green hover:border-2 group hover:border-greenDark hover:shadow-lg">
                      <div className="col-span-9 grid grid-cols-12 gap-3 items-center group-hover:text-greenDark">
                        ...
                        <div className="col-span-9 text-sm">
                          <p className="text-lg">{item.namaLatin}</p>
          ...
          <div className="text-center text-3xl text-greenDark py-32">
