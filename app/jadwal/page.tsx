...
const JadwalSholat = () => {
  ...
  const getSchedule = async (code: string) => {
    try {
      const response = await fetch(
        `https://api.myquran.com/v1/sholat/jadwal/${code}/${scheduleTime.year}/${scheduleTime.month}`
      );
      const data = await response.json();
      if (data) {
        setDatas(data.data);
  ...
  useEffect(() => {
    const value = cities.filter((item) =>
      filter.includes(item.name.toUpperCase())
    );
    setCities(value);
  }, [filter]);
  ...
  return (
    <div className="flex flex-col justify-center gap-5">
      ...
      <div className="w-full bg-greenDark text-white p-12 text-center rounded-t-xl text-3xl max-md:text-xl max-md:p-5">
        <h1>
          JADWAL SHOLAT WILAYAH {datas?.lokasi ? datas.lokasi : "Kebumen"} dan
          SEKITARNYA
        </h1>
        <p>
          {months[parseInt(scheduleTime.month) - 1]}{" "}
          ...
          <tbody className="text-center">
            {datas?.jadwal.map((item, index) => {
              return (
                <tr
                  key={index}
                  className={`${index % 2 == 0 ? "bg-grayLight" : "bg-white"}`}
                >
                  <td>{item.date}</td>
                  <td>{item.imsak}</td>
                  <td>{item.subuh}</td>
